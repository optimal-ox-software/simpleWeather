...
const COMPILER_BUILD = {
    id: '20190926165042',
    minfyJs: 'terser4.3.1_3',
    optimizeCss: 'autoprefixer9.6.1_cssnano4.1.10_postcss7.0.18_3',
...
const getAttributeName = (_diagnostics, propName, propOptions) => {
...
const gatherVdomMeta = (m, args) => {
    ...
    if (args[0].kind === ts$1__default.SyntaxKind.Identifier) {
        m.hasVdomFunctional = true;
    }
    if (args.length > 1) {
        const objectLiteral = args[1];
        if (ts$1__default.isObjectLiteralExpression(objectLiteral)) {
        ...
        if (!m.hasVdomText) {
            for (let i = 2; i < args.length; i++) {
                if (ts$1__default.isStringLiteral(args[i])) {
                    m.hasVdomText = true;
                    break;
                }
            }
