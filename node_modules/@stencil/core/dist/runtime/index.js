        ...
        if (typeof style === 'string') {
            ...
            if (!appliedStyles.has(scopeId)) {
                if (buildConditionals.BUILD.hydrateClientSide && styleContainerNode.host && (styleElm = styleContainerNode.firstElementChild) && styleElm.tagName === 'STYLE') {
                    ...
                    if (platform.cssVarShim) {
                        styleElm = platform.cssVarShim.createHostStyle(hostElm, scopeId, style, !!(cmpMeta.$flags$ & 10 /* needsScopedEncapsulation */));
...
const setAccessor = (elm, memberName, oldValue, newValue, isSvg, flags) => {
    ...
    if (buildConditionals.BUILD.vdomClass && memberName === 'class') {
        ...
        else if (platform.isMemberInElement(elm, ln)) {
            // standard event
...
const createElm = (oldParentVNode, newParentVNode, childIndex, parentElm) => {
    ...
    if (buildConditionals.BUILD.isDev && newVNode.$elm$) {
        platform.consoleError(`The JSX ${utils.isDef(newVNode.$text$) ? `"${newVNode.$text$}" text` : `"${newVNode.$tag$}" element`} node should not be shared within the same renderer. The renderer caches element lookups in order to improve performance. However, a side effect from this is that the exact same JSX node should not be reused. For more information please see https://stenciljs.com/docs/templating-jsx#avoid-shared-jsx-nodes`);
    }
    if (utils.isDef(newVNode.$text$)) {
        // create text node
        newVNode.$elm$ = platform.doc.createTextNode(newVNode.$text$);
    }
    else if (buildConditionals.BUILD.slotRelocation && newVNode.$flags$ & 1 /* isSlotReference */) {
        // create a slot reference node
        newVNode.$elm$ = (buildConditionals.BUILD.isDebug || buildConditionals.BUILD.hydrateServerSide) ? platform.doc.createComment(`slot-reference:${hostTagName.toLowerCase()}`) : platform.doc.createTextNode('');
    }
    else {
        // create element
        elm = newVNode.$elm$ = ((buildConditionals.BUILD.svg && (isSvgMode || newVNode.$tag$ === 'svg'))
            ? platform.doc.createElementNS(utils.SVG_NS, newVNode.$tag$)
...
const removeVnodes = (vnodes, startIdx, endIdx, elm) => {
    for (; startIdx <= endIdx; ++startIdx) {
        if (utils.isDef(vnodes[startIdx])) {
        ...
        if (oldStartVnode == null) {
            ...
            if (buildConditionals.BUILD.vdomKey) {
                for (i = oldStartIdx; i <= oldEndIdx; ++i) {
                    if (oldCh[i] && utils.isDef(oldCh[i].$key$) && oldCh[i].$key$ === newStartVnode.$key$) {
                        idxInOld = i;
                        break;
                    }
                }
...
const patch = (oldVNode, newVNode) => {
    ...
    if (!utils.isDef(newVNode.$text$)) {
        ...
        if (buildConditionals.BUILD.updatable && utils.isDef(oldChildren) && utils.isDef(newChildren)) {
            // looks like there's child vnodes for both the old and new vnodes
            updateChildren(elm, oldChildren, newVNode, newChildren);
        }
        else if (utils.isDef(newChildren)) {
            // no old child vnodes, but there are new child vnodes to add
            if (buildConditionals.BUILD.updatable && buildConditionals.BUILD.vdomText && utils.isDef(oldVNode.$text$)) {
        ...
        else if (buildConditionals.BUILD.updatable && utils.isDef(oldChildren)) {
...
const scheduleUpdate = (elm, hostRef, cmpMeta, isInitialLoad) => {
    ...
    if (isInitialLoad) {
        ...
        if (buildConditionals.BUILD.hostListener && hostRef.$queuedListeners$) {
...
const updateComponent = (elm, hostRef, cmpMeta, instance, isInitialLoad) => {
    ...
    if (platform.cssVarShim) {
        platform.cssVarShim.updateHost(elm);
    }
...
const initializeComponent = async (elm, hostRef, cmpMeta, hmrVersionId, Cstr) => {
    ...
    if (buildConditionals.BUILD.lifecycle && buildConditionals.BUILD.lazyLoad && ancestorComponent && ancestorComponent['s-lr'] === false && ancestorComponent['s-rc']) {
        ...
        if (!(hostRef.$flags$ & 1 /* hasConnected */)) {
            ...
            if (buildConditionals.BUILD.lifecycle && buildConditionals.BUILD.lazyLoad) {
                ...
                while ((ancestorComponent = (ancestorComponent.parentNode || ancestorComponent.host))) {
                    ...
                    if ((buildConditionals.BUILD.hydrateClientSide && ancestorComponent.nodeType === 1 /* ElementNode */ && ancestorComponent.hasAttribute('s-id')) || (ancestorComponent['s-init'] && ancestorComponent['s-lr'] === false)) {
...
const setContentReference = (elm, contentRefElm) => {
    ...
    // create a node to represent where the original
    // content was first placed, which is useful later on
    contentRefElm = elm['s-cr'] = platform.doc.createComment(buildConditionals.BUILD.isDebug ? `content-ref:${elm.tagName}` : '');
...
const disconnectedCallback = (elm) => {
    if ((platform.plt.$flags$ & 1 /* isTmpDisconnected */) === 0) {
        ...
        if (platform.cssVarShim) {
            platform.cssVarShim.removeHost(elm);
        }
...
const bootstrapLazy = (lazyBundles, options = {}) => {
    ...
    lazyBundles.forEach(lazyBundle => lazyBundle[1].forEach(compactMeta => {
        ...
        const HostElement = class extends HTMLElement {
            ...
            's-init'() {
                const hostRef = platform.getHostRef(this);
...
const createEvent = (ref, name, flags) => {
    ...
    return {
        emit: (detail) => {
            ...
            return elm.dispatchEvent(new (buildConditionals.BUILD.hydrateServerSide ? platform.win.CustomEvent : CustomEvent)(name, {
