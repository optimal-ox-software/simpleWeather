...
const patchEsm = () => {
    ...
    if (!(win.CSS && win.CSS.supports && win.CSS.supports('color', 'var(--c)'))) {
        // @ts-ignore
        return new Promise(function (resolve) { resolve(require('./css-shim-cd2420a0.js')); });
    }
...
const patchBrowser = async () => {
    ...
    if (importMeta !== '') {
        ...
        if (!window.customElements) {
            // @ts-ignore
            await new Promise(function (resolve) { resolve(require('./dom-50d3d464.js')); });
        }
...
const createElm = (oldParentVNode, newParentVNode, childIndex, parentElm) => {
    ...
    if (isDef(newVNode.$text$)) {
        ...
        newVNode.$elm$ = (buildConditionals.BUILD.isDebug || buildConditionals.BUILD.hydrateServerSide) ? doc.createComment(`slot-reference:${hostTagName.toLowerCase()}`) : doc.createTextNode('');
    }
    else {
        // create element
...
const setContentReference = (elm, contentRefElm) => {
    ...
    // create a node to represent where the original
    // content was first placed, which is useful later on
    contentRefElm = elm['s-cr'] = doc.createComment(buildConditionals.BUILD.isDebug ? `content-ref:${elm.tagName}` : '');
